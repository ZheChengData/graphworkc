...
[cost_matrix_to_numpy]函数根据输入的起点列表和终点列表，计算生成获得一个开销矩阵
...
end_nodes_list = [3, 7]
s_matrix = g.cost_matrix_to_numpy(start_nodes_list, end_nodes_list,
...
[path_list_to_dict]函数根据输入的起点列表和终点列表，计算生成获得一个路径字典：

```python
start_nodes_list = [1, 4]
end_nodes_list = [3, 7]
s_list = g.path_list_to_dict(start_nodes_list, end_nodes_list, 
                             weight_name="l", num_thread=10)
print(s_list)
...
[shortest_paths]函数根据输入的**起点、终点**返回两点之间**最短的前K条路径**：

假设有如下图：则可通过此函数获取1到5的三条最短路径
<figure markdown="span">
  ![Image title](./img/k_path.png){ width="250" }
  <figcaption>K最短路</figcaption>
</figure>

```python

res = g.shortest_paths(1, 7, 3, weight_name="l")
print(res)
